...
describe("ClaimMessage · 98-byte canonical encoding (S2-T3)", () => {
  ...
  it("layout: [34..66] are quizStateHash raw bytes from hex", () => {
    ...
    expect(Buffer.from(bytes.subarray(34, 66))).toEqual(
  ...
  it("layout: [82..98] are nonce raw bytes from hex", () => {
    ...
    expect(Buffer.from(bytes.subarray(82, 98))).toEqual(
...
describe("ClaimMessage · ed25519 sign/verify (S2-T3)", () => {
  it("sign+verify roundtrip with matching keypair returns true", () => {
    ...
    expect(
      verifyClaimSignature(signed.messageBytes, signed.signature, signed.signerPubkey),
    ).toBe(true)
  })
  ...
  it("verify rejects modified message bytes (single bit flip)", () => {
    ...
    expect(verifyClaimSignature(tampered, signed.signature, signed.signerPubkey)).toBe(
      false,
    )
  })
  ...
  it("verify rejects modified signature (single bit flip)", () => {
    ...
    tampered[0] ^= 0x01
    expect(
      verifyClaimSignature(signed.messageBytes, tampered, signed.signerPubkey),
    ).toBe(false)
  })
  ...
  it("verify rejects wrong signer pubkey (bound to different secret)", () => {
    ...
    const signed = signClaimMessage(makeClaim(), secret)
    expect(
      verifyClaimSignature(signed.messageBytes, signed.signature, wrongSigner.pubkey),
    ).toBe(false)
  })
  ...
  it("rejects secret of wrong length", () => {
    expect(() => signClaimMessage(makeClaim(), randomBytes(32))).toThrow(
      /must be 64 bytes/,
    )
    expect(() => signClaimMessage(makeClaim(), randomBytes(128))).toThrow(
      /must be 64 bytes/,
    )
  })
  ...
  it("verifyClaimSignature returns false on length-mismatched inputs (defense)", () => {
    ...
    expect(
      verifyClaimSignature(new Uint8Array(50), signed.signature, signed.signerPubkey),
    ).toBe(false)
    // Wrong-length signature
    expect(
      verifyClaimSignature(signed.messageBytes, new Uint8Array(63), signed.signerPubkey),
    ).toBe(false)
    // Wrong-length pubkey
    expect(
      verifyClaimSignature(signed.messageBytes, signed.signature, new Uint8Array(31)),
    ).toBe(false)
    // Sanity: matched lengths still verify
    expect(
      verifyClaimSignature(signed.messageBytes, signed.signature, pubkey),
    ).toBe(true)
  })
