  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-puru-cloud-deep/40">
      <div className="flex items-center gap-3">
        <span className={`px-2.5 py-1 rounded-full text-2xs font-puru-mono uppercase tracking-wide ${ELEMENT_TINT_BG[weather]} text-puru-ink-rich`}>
          {ELEMENT_META[weather].kanji} {ELEMENT_META[weather].name}
        </span>
        <span className="text-xs font-puru-body text-puru-ink-soft">
          today's tide
        </span>
      </div>

      <div className="flex items-center gap-2 text-xs font-puru-body text-puru-ink-soft">
        <span className={`px-2 py-0.5 rounded-full ${ELEMENT_TINT_BG[opponentElement]} text-puru-ink-rich font-puru-display`}>
          {condition.name}
        </span>
