...
import {
  checkPayerBalance,
  loadSponsoredPayer,
} from "@/lib/blink/sponsored-payer"
...
const freshNonce = (): ClaimNonce =>
  randomBytes(16).toString("hex") as ClaimNonce
...
export async function POST(request: Request) {
  ...
  if (error) {
    return errorResponse(
      "We couldn't read your answers · please take the quiz again.",
      400,
      error,
    )
  }
  ...
  const quizStateHash = quizStateHashOf(
    answers as ReadonlyArray<0 | 1 | 2 | 3>,
  ) as QuizStateHash
  ...
  if (nonceResult === "replay") {
    return errorResponse(
      "Already claimed · check your wallet.",
      409,
  ...
  if (!balance.canSponsor) {
    console.error(
      `[mint-error] sponsored-payer balance below threshold · ${balance.sol} SOL`,
    )
    return errorResponse(
      "Network's briefly out of reach · please try again.",
      503,
