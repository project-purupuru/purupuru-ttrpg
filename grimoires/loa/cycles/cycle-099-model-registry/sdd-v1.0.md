...
**Version:** 1.0 (initial — pre-Flatline)
**Date:** 2026-05-04
**Author:** Architecture Designer (Claude Opus 4.7 1M)
**Status:** Draft — ready for SDD-level Flatline review (cycle-098 pattern: 2-4 passes with HIGH_CONSENSUS auto-integration)
...
3. Cycle-100 schedule: deletion of the standalone wrapper. Migration runbook at `grimoires/loa/runbooks/model-permissions-removal.md` (Sprint 3 deliverable).

**Rollback path:** if Sprint 3 acceptance reveals tooling that hard-depends on `model-permissions.yaml` being editable (not generated), set `LOA_LEGACY_MODEL_PERMISSIONS=1` env var to restore cycle-098 behavior (separate hand-maintained file). Documented in same runbook.
...
|---------|------|----------|
| `.run/merged-model-aliases.sh` absent | `[MERGED-ALIASES-MISSING]` | Regenerate via startup hook |
| Bash syntax check fails on regen output | `[MERGED-ALIASES-CORRUPT]` | Refuse to start (fail-closed) |
| Write permission failure / disk full | `[MERGED-ALIASES-WRITE-FAILED]` | Refuse to start (fail-closed) |
| SHA256 mismatch (stale per FR-1.9) | `[MERGED-ALIASES-STALE]` | Regenerate under exclusive flock |
| flock acquisition timeout (>30s) | `[MERGED-ALIASES-LOCK-TIMEOUT]` | Refuse to start (fail-closed); operator runbook for stuck-lock recovery |
| Network-fs detected without opt-in | `[MERGED-ALIASES-NETWORK-FS]` | Refuse to start; operator must `LOA_ALLOW_NETWORK_FS_FOR_MERGED_ALIASES=1` (resolves SKP-005) |
| Schema-invalid `model_aliases_extra` (caught at load before hook write) | `[MODEL-EXTRA-INVALID]` | Refuse to start |
...
### 6.5 URL Canonicalization Rules (resolves SKP-003)
...
| 5 | If hostname looks like IDN (contains non-ASCII OR `xn--` prefix): convert via `idna.encode()` (Python `idna` library, already a transitive dep of `requests`). Then re-validate against allowlist. Reject mixed-script labels (Unicode TR-39 confusables). | `[ENDPOINT-IDN-CONFUSABLE]` |
| 6 | Punycode normalization: a domain like `xn--googl-fsa.com` (looks like google but isn't) is checked against allowlist as-decoded. Reject punycode that decodes to a string within Levenshtein distance <2 from an allowlist host. | `[ENDPOINT-PUNYCODE-LOOKALIKE]` |
| 7 | `port` rules: if absent, infer 443 for HTTPS. If present, MUST equal 443 for OpenAI/Anthropic/Google; allowlist per-provider for Bedrock (some regional endpoints use non-default ports). Reject non-default ports unless explicitly permitted in `loa.defaults.yaml::providers.<p>.allowed_endpoints[].ports`. | `[ENDPOINT-PORT-NOT-ALLOWED]` |
| 8 | `path` normalization: reject `..`, `./`, repeated slashes, percent-encoded path-traversal (`%2e%2e/`), Unicode normalization tricks (right-to-left override). | `[ENDPOINT-PATH-INVALID]` |
| 9 | Final check: rebuild canonical URL; the canonical form MUST match the schema-validated allowlist host pattern exactly. Match is on the IDNA-normalized + lowercased hostname. | `[ENDPOINT-NOT-ALLOWED]` |

**Test corpus** (`tests/integration/url-canonicalization.bats`): probes for every step above, including IPv6 literal `https://[::1]/v1`, IDN homograph `https://googⅬe.com/v1` (Cyrillic L), punycode `xn--ggle-vqa.com`, port `:8080`, path traversal `https://api.openai.com/../v1`, RTL override.
...
**Acceptance criteria**: SC-1 (partial — only registries 1, 2, 4, 5 covered), SC-8 (drift gate green).
...
|----------|-------|----------|--------|
| Should `tier_groups.mappings.tiny` for `openai` use `gpt-5.3-codex` (no smaller option) or fail-closed when `tiny` is requested for OpenAI? | deep-name | Sprint 2 design — 2026-05-11 | Open. SDD §3.1.2 default uses `gpt-5.3-codex` per "no smaller alternative; fail-closed would surprise". Operator override always available. |
| Should `model_aliases_override` allow overriding `endpoint_family`? | deep-name | Sprint 2 design — 2026-05-11 | Open. SDD §3.3 currently forbids implicitly (only fields explicitly in §3.3 examples shown). Sprint 2 schema needs explicit allowlist. |
| Should DD-1 Option B `permissions` block be REQUIRED for every model in `model-config.yaml` post-Sprint 3, or remain OPTIONAL with FR-1.4 minimal-baseline fallback? | deep-name | Sprint 3 design — 2026-05-18 | Open. Current SDD treats as OPTIONAL (back-compat). Required-for-all is Sprint 3 stretch goal. |
| Should `model-overlay-hook.py` be invoked via Python venv (existing cheval venv) or via system Python? | deep-name | Sprint 2 implementation — 2026-05-11 | Open. SDD §1.4.4 currently says "reuses cheval's PyYAML"; whether that means same venv or system Python with separate install needs clarification at impl time. |
| Should we expose a `loa doctor model` subcommand to combine all SDD validation checks (lockfile + drift + bindings + endpoint allowlist + flock-fs)? | deep-name | Sprint 2 stretch — 2026-05-11 | Open. Useful for operator one-line health check; not strictly required by PRD. |
...
- Unicode TR-39 (Confusable Characters; used by §6.5 step 5): https://www.unicode.org/reports/tr39/
- Python `urllib.parse` docs: https://docs.python.org/3/library/urllib.parse.html
- Python `idna` library: https://pypi.org/project/idna/
- POSIX `flock(2)` semantics: https://man7.org/linux/man-pages/man2/flock.2.html

### C. Resolution of PRD Deferred Decisions and SDD-Shape Findings

| ID | Decision | SDD section | Resolution |
|----|----------|-------------|------------|
| **DD-1** | model-permissions strategy | §3.1.1, §5.3, §8 Sprint 3 | **Option B** (merge into model-config.yaml). 7-dim trust_scopes preserved as nested structure. Sprint 3 ships `gen-model-permissions.sh` thin wrapper for cycle-099 back-compat; cycle-100 deletes wrapper. |
| **DD-2** | model_aliases_extra placement | §3.2, §4.1 | **Top-level** (mirrors `protected_classes_extra` cycle-098 pattern). |
| **DD-3** | model_aliases_override semantics | §3.3 | **Partial-merge override** at depth 2; explicit-fields-win; silent override REJECTED; `permissions` override forbidden for framework defaults. |
| **DD-4** | startup hook language | §1.4.4, §2.1 | **Python** (cheval is already Python; reuses PyYAML; jsonschema 4.21+). |
| **DD-5** | JSON Schema location + validator language | §3.2 | Schema at `.claude/data/trajectory-schemas/model-aliases-extra.schema.json`. Python validator (matches DD-4); ajv-TS for Bridgebuilder side. |
| **DD-6** | Property test runner | §2.2, §7.1 | **bats + simple bash property generator** at `tests/property/lib/property-gen.bash` (~150 lines). 0 new dependencies. Stretch swap to Hypothesis if state-space coverage proves insufficient. |
| **SKP-003** | URL canonicalization edge cases | §6.5 | 9-step canonicalization pipeline; rejection codes; `tests/integration/url-canonicalization.bats`. |
| **SKP-004** | shell-escape safety in merged-aliases.sh | §3.5 | shlex.quote() per value; key/value validation pre-emit; abort on shell metachar; `tests/integration/merged-aliases-shell-escape.bats`. |
| **SKP-005** | flock semantics over network filesystems | §6.6, §1.9 | filesystem-type detection blocklist; refuse on NFS/SMB/etc unless `LOA_ALLOW_NETWORK_FS_FOR_MERGED_ALIASES=1`; runbook + WARN log on override. |
| **SKP-006** | hybrid BB TS-runtime/runtime-overlay divergence | §1.4.6 | shallow-copy compiled + per-key operator overlay; one-time-per-process `[BB-OVERLAY-OVERRIDE]` emission per overridden key; `tests/integration/bb-runtime-overlay-divergence.bats`. |
...
*Generated by Architecture Designer Agent. Cycle-099 model-registry consolidation. Operator priority: URGENT.*
