...
function dropletXY(kind: number): { x: number; y: number } {
  ...
  switch (kind & 3) {
    case 0: return { x: CX + 18, y: CY - 14 };  // top-right
    case 1: return { x: CX + 16, y: CY + 16 };  // bottom-right
    case 2: return { x: CX - 18, y: CY + 14 };  // bottom-left
    default: return { x: CX - 16, y: CY - 16 }; // top-left
  }
}
...
function eyesSVG(kind: number, tilt: number): string {
  ...
  switch (kind & 7) {
    case 0: // open round
      return `<circle cx="${L.x}" cy="${L.y}" r="2.2" fill="${INK_HEX}"/>` +
             `<circle cx="${R.x}" cy="${R.y}" r="2.2" fill="${INK_HEX}"/>`;
    case 1: // narrow happy ⌣ (upward arcs)
      return `<path d="M ${L.x - 2.5} ${L.y + 1} Q ${L.x} ${L.y - 2} ${L.x + 2.5} ${L.y + 1}" stroke="${INK_HEX}" stroke-width="1.6" stroke-linecap="round" fill="none"/>` +
    ...
    case 2: // sleepy (lines + tiny dots)
      return `<line x1="${L.x - 2.5}" y1="${L.y}" x2="${L.x + 2.5}" y2="${L.y}" stroke="${INK_HEX}" stroke-width="1.4" stroke-linecap="round"/>` +
             `<line x1="${R.x - 2.5}" y1="${R.y}" x2="${R.x + 2.5}" y2="${R.y}" stroke="${INK_HEX}" stroke-width="1.4" stroke-linecap="round"/>`;
    case 3: // surprised
      return `<circle cx="${L.x}" cy="${L.y}" r="2.8" fill="none" stroke="${INK_HEX}" stroke-width="1.2"/>` +
    ...
    default: { // half-closed (downward arcs)
      void tilt;
      return `<path d="M ${L.x - 2.5} ${L.y - 1} Q ${L.x} ${L.y + 1.5} ${L.x + 2.5} ${L.y - 1}" stroke="${INK_HEX}" stroke-width="1.6" stroke-linecap="round" fill="none"/>` +
...
function mouthSVG(kind: number): string {
  ...
  switch (kind & 7) {
    ...
    default: // drool / open
      return `<path d="M ${M.x - 3.5} ${M.y} Q ${M.x} ${M.y + 3} ${M.x + 3.5} ${M.y}" stroke="${INK_HEX}" stroke-width="1.5" stroke-linecap="round" fill="none"/>` +
...
function browSVG(tilt: -1 | 0 | 1): string {
  ...
  return `<line x1="${L.x - 3}" y1="${L.y - 5 + dy}" x2="${L.x + 2}" y2="${L.y - 5 - dy}" stroke="${INK_HEX}" stroke-width="1.3" stroke-linecap="round"/>` +
...
function dropletSVG(kind: number): string {
  ...
  return `<path d="M ${p.x} ${p.y - 3} Q ${p.x - 2} ${p.y} ${p.x} ${p.y + 2} Q ${p.x + 2} ${p.y} ${p.x} ${p.y - 3} Z" fill="${HONEY_HEX}" opacity="0.9"/>` +
...
function drawEyes(ctx: CanvasRenderingContext2D, kind: number): void {
  ...
  switch (kind & 7) {
    ...
    case 1:
      ...
      ctx.beginPath(); ctx.moveTo(L.x - 2.5, L.y + 1); ctx.quadraticCurveTo(L.x, L.y - 2, L.x + 2.5, L.y + 1); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(R.x - 2.5, R.y + 1); ctx.quadraticCurveTo(R.x, R.y - 2, R.x + 2.5, R.y + 1); ctx.stroke();
      break;
    case 2:
      setStrokeBase(ctx, 1.4);
      ctx.beginPath(); ctx.moveTo(L.x - 2.5, L.y); ctx.lineTo(L.x + 2.5, L.y); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(R.x - 2.5, R.y); ctx.lineTo(R.x + 2.5, R.y); ctx.stroke();
      break;
    case 3:
      setStrokeBase(ctx, 1.2);
      ctx.beginPath(); ctx.arc(L.x, L.y, 2.8, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(R.x, R.y, 2.8, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = INK_HEX;
      ctx.beginPath(); ctx.arc(L.x, L.y, 1.2, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(R.x, R.y, 1.2, 0, Math.PI * 2); ctx.fill();
      break;
    default:
      setStrokeBase(ctx, 1.6);
      ctx.beginPath(); ctx.moveTo(L.x - 2.5, L.y - 1); ctx.quadraticCurveTo(L.x, L.y + 1.5, L.x + 2.5, L.y - 1); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(R.x - 2.5, R.y - 1); ctx.quadraticCurveTo(R.x, R.y + 1.5, R.x + 2.5, R.y - 1); ctx.stroke();
  }
}
...
function drawMouth(ctx: CanvasRenderingContext2D, kind: number): void {
  ...
  switch (kind & 7) {
    case 0:
      ctx.beginPath(); ctx.moveTo(M.x - 4, M.y - 1); ctx.quadraticCurveTo(M.x, M.y + 3, M.x + 4, M.y - 1); ctx.stroke();
      break;
    case 1:
      ctx.beginPath(); ctx.moveTo(M.x - 3, M.y); ctx.lineTo(M.x + 3, M.y); ctx.stroke();
      break;
    case 2:
      ctx.beginPath(); ctx.moveTo(M.x - 4, M.y);
    ...
    case 3:
      ctx.fillStyle = INK_HEX;
      ctx.beginPath(); ctx.ellipse(M.x, M.y + 0.5, 2, 2.4, 0, 0, Math.PI * 2); ctx.fill();
      break;
    default:
      ctx.beginPath(); ctx.moveTo(M.x - 3.5, M.y); ctx.quadraticCurveTo(M.x, M.y + 3, M.x + 3.5, M.y); ctx.stroke();
      ctx.fillStyle = HONEY_HEX;
      ctx.beginPath(); ctx.arc(M.x + 3, M.y + 4.5, 1.2, 0, Math.PI * 2); ctx.fill();
  }
}
...
function drawBrows(ctx: CanvasRenderingContext2D, tilt: -1 | 0 | 1): void {
  ...
  ctx.beginPath(); ctx.moveTo(L.x - 3, L.y - 5 + dy); ctx.lineTo(L.x + 2, L.y - 5 - dy); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(R.x - 2, R.y - 5 - dy); ctx.lineTo(R.x + 3, R.y - 5 + dy); ctx.stroke();
}
...
function drawDroplet(ctx: CanvasRenderingContext2D, kind: number): void {
  ...
  ctx.beginPath(); ctx.arc(p.x - 0.6, p.y - 0.6, 0.6, 0, Math.PI * 2); ctx.fill();
}
...
export function avatarToCanvas(
  ...
  ctx.beginPath(); ctx.arc(CX + 12, CY + 10, 3.2, 0, Math.PI * 2); ctx.fill();
  ctx.globalAlpha = 1;

  // top-left highlight
  ctx.fillStyle = "rgba(255,255,255,0.20)";
  ctx.beginPath(); ctx.ellipse(CX - 8, CY - 12, 9, 5, 0, 0, Math.PI * 2); ctx.fill();
