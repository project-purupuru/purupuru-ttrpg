...
function mockGit(overrides) {
    return {
        ...
        getPRFiles: async () => [
            { filename: "src/app.ts", status: "modified", additions: 5, deletions: 3, patch: "+code" },
        ],
        getPRReviews: async () => [],
...
function mockHasher() {
    return { sha256: async (input) => `hash-${input.slice(0, 10)}` };
}
...
function mockSanitizer(overrides) {
    return {
        sanitize: (content) => ({
            safe: true,
...
function buildPipeline(opts) {
    ...
    return new ReviewPipeline(template, context, git, mockPoster(opts?.poster), mockLLM(opts?.llm), mockSanitizer(opts?.sanitizer), opts?.logger ?? mockLogger(), "You are a code reviewer.", config, opts?.now ?? Date.now);
}
describe("ReviewPipeline", () => {
        ...
        it("skips remaining items when runtime limit exceeded", async () => {
            ...
            const pipeline = buildPipeline({
                ...
                git: {
                    ...
                    getPRFiles: async () => [
                        { filename: "f.ts", status: "modified", additions: 1, deletions: 0, patch: "+x" },
                    ],
                    getPRReviews: async () => [],
                    preflight: async () => ({ remaining: 5000, scopes: ["repo"] }),
            ...
            const summary = await pipeline.run("run-1");
            const runtimeSkipped = summary.results.filter((r) => r.skipReason === "runtime_limit");
    ...
    describe("RunSummary counts", () => {
        it("returns accurate reviewed/skipped/errors counts", async () => {
            ...
            assert.equal(summary.reviewed + summary.skipped + summary.errors, summary.results.length);
        });
    });
    describe("token calibration logging (BB-F1)", () => {
        it("emits calibration log with ratio when inputTokens available", async () => {
            const infoCalls = [];
            const logger = {
                info: (msg, data) => { infoCalls.push({ msg, data: data ?? {} }); },
                warn: () => { },
                error: () => { },
                debug: () => { },
            };
            ...
            assert.ok(calibrationLog.data.ratio > 0, "Ratio should be positive");
        ...
        it("reviews only delta files when lastReviewedSha exists", async () => {
            ...
            const pipeline = buildPipeline({
                git: {
                    ...
                    getPRFiles: async () => [
                        { filename: "src/app.ts", status: "modified", additions: 5, deletions: 3, patch: "+code" },
                        { filename: "src/old.ts", status: "modified", additions: 1, deletions: 1, patch: "+old" },
                    ],
                    getCommitDiff: async () => ({
        ...
        it("two-pass handles all-files-excluded by Loa filtering", async () => {
            ...
            const pipeline = buildPipeline({
                ...
                git: {
                    ...
                    getPRFiles: async () => [
                        { filename: ".claude/loa/something.md", status: "modified", additions: 5, deletions: 3, patch: "+code" },
                    ],
                    getPRReviews: async () => [],
                    preflight: async () => ({ remaining: 5000, scopes: ["repo"] }),
                    preflightRepo: async () => ({ owner: "test", repo: "repo", accessible: true }),
