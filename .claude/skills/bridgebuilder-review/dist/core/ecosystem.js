 ...
 */
export function extractEcosystemPatterns(findings, repo, prNumber) {
    const patterns = [];
    for (const finding of findings) {
        const severity = (finding.severity ?? "").toUpperCase();
        const isPraiseHighConfidence = severity === "PRAISE" &&
 ...
 */
export async function updateEcosystemContext(contextPath, newPatterns, logger) {
    try {
        ...
        const existingKeys = new Set(context.patterns.map((p) => `${p.repo}::${p.pattern}`));
        ...
        if (indicesToRemove.size > 0) {
            context.patterns = context.patterns.filter((_, idx) => !indicesToRemove.has(idx));
        }
        // Update timestamp
