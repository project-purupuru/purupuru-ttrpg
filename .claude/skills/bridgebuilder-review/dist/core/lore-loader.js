 ...
 */
function coerceLoreEntry(raw, index, logger) {
    ...
    if (typeof r.id !== "string" || typeof r.term !== "string" ||
        ...
        logger?.warn(`[lore-loader] Entry ${index} missing required field(s) (id/term/short/context) — skipping`);
        return null;
    }
    ...
    if (typeof r.source === "string") {
        ...
        const s = r.source;
        const bits = [];
        if (typeof s.cycle === "string")
            bits.push(s.cycle);
        if (typeof s.bridge_iteration === "string")
            bits.push(s.bridge_iteration);
 ...
 */
export async function loadLoreEntries(path = DEFAULT_LORE_PATH, logger) {
    if (!existsSync(path)) {
    ...
    let stdout;
    try {
        const result = await execFileAsync("yq", ["-o", "json", "eval", ".", path], { timeout: YQ_TIMEOUT_MS });
        stdout = result.stdout;
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        throw new Error(`[lore-loader] Failed to convert YAML at ${path}: ${message}. ` +
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        throw new Error(`[lore-loader] yq produced invalid JSON for ${path}: ${message}`);
    }
    if (!Array.isArray(parsed)) {
        logger?.warn(`[lore-loader] Expected top-level YAML array at ${path}, got ${typeof parsed} — returning empty entries`);
        return [];
    }
    const entries = [];
...
//# sourceMappingURL=lore-loader.js.map