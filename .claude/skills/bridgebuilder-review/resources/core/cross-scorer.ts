 ...
 */
import type { ModelFindings, ScoredFinding } from "./scoring.js";
 ...
 */
function comparePair(a: ModelFindings, b: ModelFindings): PairwiseComparison {
  const agreements: PairwiseComparison["agreements"] = [];
  const disagreements: PairwiseComparison["disagreements"] = [];
  ...
  for (let j = 0; j < b.findings.length; j++) {
    if (matchedB.has(j) || outputChars >= MAX_OUTPUT_CHARS) continue;
...
function findingSimilarity(
  a: { file?: string; category: string; title: string; description: string },
  b: { file?: string; category: string; title: string; description: string },
): number {
  let score = 0;
  let weight = 0;
